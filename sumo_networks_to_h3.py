...
Outputs:
  1. edge_hex_mapping.csv      one row per edge-H3-cell relation
  2. edges.geojson             network edge geometries in lon/lat
  3. hexes.geojson             H3 hex polygons with edge counts
  4. network_hex_map.html      interactive Folium map

Example:
  python sumo_networks_to_h3.py \
    --clone-repo \
    --resolution 10 \
    --buffer-m 20 \
    --out out_h3

Alternative, after manually cloning the repo:
  python sumo_networks_to_h3.py \
...
def h3_geo_to_cells(geo, res: int) -> set[str]:
    ...
    In h3-py v4, geo_to_cells accepts objects implementing __geo_interface__.
    """
...
def utm_crs_from_lonlat(lon: float, lat: float) -> CRS:
    ...
    Good enough for city/regional SUMO networks.
    """
...
def main() -> None:
    ...
    print("\nDone.")
    print(f"Edge-H3 mapping CSV: {mapping_csv}")
